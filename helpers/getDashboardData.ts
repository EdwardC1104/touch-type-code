import fetchLessonResults from "data/fetchLessonResults";
import fetchLessonsWithBestResults from "data/fetchLessonsWithBestResults";
import dayjs from "dayjs";
import averageList from "helpers/averageList";
...
const getDashboardData = async (userId: string) => {
  ...
  const bestResults = await fetchLessonsWithBestResults(userId);

  // Get all the results a user has achieved
  const results = await fetchLessonResults(userId);
